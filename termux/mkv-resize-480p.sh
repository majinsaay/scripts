...
for file in $tocom; do
  ...
  if [ "$width" -gt "$height" ]; then
    ffmpeg -i "$file" -map 0 -c:v libx264 -preset veryfast -crf 22 -vf "format=yuv420p,scale=-2:480" -c:a libopus -b:a 128k -c:s copy "${file%.*}.temp.mkv"
  else
    ffmpeg -i "$file" -map 0 -c:v libx264 -preset veryfast -crf 22 -vf "format=yuv420p,scale=480:-2" -c:a libopus -b:a 128k -c:s copy "${file%.*}.temp.mkv"
  fi
