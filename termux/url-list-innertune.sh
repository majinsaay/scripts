...
# generate url list for all your music from innertune backup db (playlists)
# also works for forks
# required: sqlite
...
for db in *.db; do
  sqlite3 -csv "${db}" "select songId,playlistId from playlist_song_map;" > playlistmap.csv
done
