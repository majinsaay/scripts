...
for file in $tocom; do
  ...
  if [ "$width" -gt "$height" ]; then
    ffmpeg -i "$file" -map 0 -movflags +faststart -c:v libx264 -preset veryfast -crf 24 -vf "format=yuv420p,scale=-2:1080" -c:a aac -b:a 160k "${file%.*}.temp.mp4"
  else
    ffmpeg -i "$file" -map 0 -movflags +faststart -c:v libx264 -preset veryfast -crf 24 -vf "format=yuv420p,scale=1080:-2" -c:a aac -b:a 160k "${file%.*}.temp.mp4"
  fi
