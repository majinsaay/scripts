...
for file in $tocom; do
  ...
  if [ "$width" -gt "$height" ]; then
    ffmpeg -i "$file" -map 0 -movflags +faststart -c:v libx264 -preset veryfast -crf 23 -vf "format=yuv420p,scale=-2:720" -c:a aac -b:a 160k "${file%.*}.temp.mp4"
  else
    ffmpeg -i "$file" -map 0 -movflags +faststart -c:v libx264 -preset veryfast -crf 23 -vf "format=yuv420p,scale=720:-2" -c:a aac -b:a 160k "${file%.*}.temp.mp4"
  fi
